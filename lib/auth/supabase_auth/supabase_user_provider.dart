...
class PlanNestSupabaseUser extends BaseAuthUser {
  PlanNestSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> planNestSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PlanNestSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
